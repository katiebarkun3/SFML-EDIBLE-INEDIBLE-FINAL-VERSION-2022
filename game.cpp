...
Game::Game() :
	...
	secrules_screen(IMAGES_FOLDER + SECRULES_SCREEN),
	...
	player_hp(GAME_FONT, 18, 13, 28, sf::Color::Yellow),
	score_text(GAME_FONT, 24, 420, 5, sf::Color::Yellow),
	final_score_text(GAME_FONT, 40, 70, 550, sf::Color(128, 0, 0)),
	snake_text(GAME_FONT, 70, 140, 350, sf::Color(225, 0, 0)),
...
void Game::check_events() {
	...
	while (window.pollEvent(event)) {
		...
		if (game_state == PLAY && event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::Up) {
			if (!player.getIsJump()) player.jump();
		}
		if (event.type == sf::Event::KeyPressed) {
			if (game_state == INTRO && event.key.code == sf::Keyboard::Space) {
				game_state = RULES;
			}
			if (game_state == RULES && event.key.code == sf::Keyboard::Enter) {
				game_state = SECRULES;
			}
			if (game_state == SECRULES && event.key.code == sf::Keyboard::Space) {
				game_state = PLAY;
			}
		}
	}
...
void Game::update() {
	size_t num = GREEN_POTION_CLOCK;
	size_t numdeath = RED_POTION_CLOCK;
	...
	case PLAY:
		...
		snake.update();
		if (snake.getPosition().x == SNAKE_MIN_POSITION) {
			snake_text.update("");
		}
		...
		if (back12.getDel()) { window.draw(back2.getSprite()); }
...
void Game::draw() {
	...
	case SECRULES:
		window.clear(sf::Color(150, 150, 150));
		window.draw(secrules_screen.getSprite());
		break;
	case PLAY:
		window.clear(sf::Color(150, 150, 150));
		window.draw(back1.getSprite());
		back12.draw(window);
		if (score >= BACKGROUND_CHANGING_1) {
			back23.draw(window);
		}
		if (score >= BACKGROUND_CHANGING_2) {
			back34.draw(window);
		}
		if (score >= BACKGROUND_CHANGING_3) {
			back45.draw(window);
		}
		if (score >= BACKGROUND_CHANGING_4) {
			back56.draw(window);
		}
		if (score >= BACKGROUND_CHANGING_5) {
			back61.draw(window);
		}
		player.draw(window);
		for (size_t i = 0; i < FOODS_QTY; i++) {
		...
		window.draw(snake_text.getText());
		hpbar.draw(window);
		break;
	case GAME_OVER:
		window.clear(sf::Color(150, 150, 150));
